...
module Deck =
  struct
    ...
    let rec getCardFromIndice i typ : (Card.t * t) =
      match typ with
      ...
      | hd :: tl ->
        if i = 0 then
          ...
          match getCardFromIndice (i - 1) tl with
          | (card, lst) -> (card, hd :: lst)


    let rec newDeckFrom typ l : t =
      match typ with
      | [] -> []
      | lst ->
        match getCardFromIndice (Random.int (l)) lst  with
        | (card, tl) -> card :: newDeckFrom tl (l - 1)

    let newDeck : t = newDeckFrom Card.all 52

    let rec toStringList typ : string =
      match typ with
      | [] -> ""
      | hd :: tl -> (Card.toString hd) ^ " " ^ toStringList tl

    let rec toStringListVerbose typ : string =
      match typ with
      | [] -> ""
    ...
    let drawCard typ : (Card.t * t) =
      match typ with
      | [] -> raise (DeckError "Error, Empty Deck") 
      | hd :: tl -> (hd, tl)
